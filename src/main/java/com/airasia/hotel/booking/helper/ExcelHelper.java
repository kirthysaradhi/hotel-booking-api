/*
 * package com.airasia.hotel.booking.helper;
 * 
 ...
 * 
 * import com.mbb.boot.bmdm.model.Tutorial;
 * 
 * public class ExcelHelper { public static String TYPE =
 * "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"; static
 * String[] HEADERs = { "Id", "Title", "Description", "Published" }; //static
 * String SHEET = "Tutorials"; static String SHEET = "Sheet1";
 * 
 ...
 * 
 * int rowIdx = 1; for (Tutorial tutorial : tutorials) { Row row =
 ...
 * 
 * public static List<Tutorial> excelToTutorials(InputStream is) { try {
 * Workbook workbook = new XSSFWorkbook(is);
 * 
 * Sheet sheet = workbook.getSheet(SHEET); Iterator<Row> rows =
 * sheet.iterator();
 * 
 * List<Tutorial> tutorials = new ArrayList<Tutorial>();
 * 
 * int rowNumber = 0; while (rows.hasNext()) { Row currentRow = rows.next();
 * 
 ...
 * cellsInRow.next();
 * 
 * switch (cellIdx) { case 0: tutorial.setId((long)
 * currentCell.getNumericCellValue()); break;
 * 
 * case 1: tutorial.setTitle(currentCell.getStringCellValue()); break;
 * 
 * case 2: tutorial.setDescription(currentCell.getStringCellValue()); break;
 * 
 * case 3: tutorial.setPublished(currentCell.getBooleanCellValue()); break;
 * 
 * default: break; }
 * 
 * cellIdx++; }
 ...
 * workbook.close();
 * 
 * return tutorials; } catch (IOException e) { throw new
 * RuntimeException("fail to parse Excel file: " + e.getMessage()); } catch
 * (Exception ex) { throw new
 * RuntimeException("fail to parse Excel file>>>kkkk: " + ex.getMessage()); } }
 * }
 */